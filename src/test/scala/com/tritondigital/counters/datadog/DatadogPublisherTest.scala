...
class DatadogPublisherTest extends PublisherTest[FakeDatadogServer, DatadogPublisher] {
  ...
  "Datadog publisher" should {
    "publish metrics after interval" in withSut() { (server, sut) =>
      ...
      server should eventuallyVerify( havePublished(Metric6Msg) )
    }
    ...
    "add common tags" in withSut(List(Tag("tag", "test"))) { (server, sut) =>
      ...
      server should eventuallyVerify( havePublished(Metric6Msg + "|#tag:test") )
    }
    ...
    "continue to publish metrics after yet another interval" in withSut() { (server, sut) =>
      ...
      server should eventuallyVerify( havePublished(Metric6Msg, Metric7Msg) )
    }
    "restarts when Datadog reports an error" in withSut() { (server, sut) =>
      ...
      server should eventuallyVerify( haveRefused(Metric7Msg) )

      server.reportNoErrors()

      sut.publish(Metric8)

      server should eventuallyVerify( havePublished(Metric6Msg, Metric8Msg) )
    }
    "connect once server becomes available" in withSut() { (server, sut) =>
      ...
      server should eventuallyVerify( havePublished(Metric8Msg) ) // We only have the one sent when successfully reconnecting
    }
    "be able to pause" in withSut() { (server, sut) =>
      ...
      server should eventuallyVerify( havePublished(Metric7Msg) )
    }
  }
